...
A personal knowledge base organized as a tree of Markdown files, meant to be edited visually in an [Obsidian](https://obsidian.md)-like editor and queried on demand by **[Gemini CLI](https://gemini.com/gemini-code)** — Anthropic's official CLI. This repository only really works alongside it: it's `gemini` running in your terminal that reads the Skill and the capture trigger here, and decides when to look something up or save it. Without Gemini CLI installed, the vault is just a plain folder of Markdown.
...
- **[Gemini CLI](https://gemini.com/gemini-code) installed** and configured (`gemini` available in your terminal) — see the official install guide at the link.
...
- **`.gemini/`**: Skill and permissions that only apply while this project is active.
- **`.gemini/`**: the "warehouse" for **user-level** Skill, Subagents, and instructions — they work in any active project, not just this one. They live here (versioned in this repo) and get mirrored into `~/.gemini/` via symlink (see below).
- **`scripts/setup-symlinks.sh`**: recreates the `.gemini/` symlinks in `~/.gemini/`.
...
Running `./scripts/setup-symlinks.sh` again is safe at any time (idempotent) — only needed when a new Skill/Subagent gets added under `.gemini/`.
...
This usually merges without conflict because the architecture already separates "engineering" (`docs/`, `.gemini/`, `scripts/`, `.gemini/`) — which only changes here, in the template — from personal content (your filled-in `MIND.md` and whatever node folders you create) — which the template never touches. The expected friction point is `.gemini/settings.json`: it accumulates permissions granted as you use it, so if you and the template change that file at the same time you can get a merge conflict there — rare, and resolves by hand without mystery (small file). More details on this decision in [docs/ARQUITETURA.md](docs/ARQUITETURA.md), section 12.

## How knowledge capture works

During any conversation with Gemini CLI, in any project, if something looks like a fact/decision/personal preference worth keeping, Gemini asks before saving it. If confirmed, it edits the right node itself (or creates a new one) and updates the index in `MIND.md`. The full logic lives in `GEMINI.md` (trigger) and `.gemini/skills/mind/SKILL.md` (procedure).

Nothing is saved without confirmation — and nothing stops you from editing files by hand at any time; Gemini only reads the current state of the files when queried.

## Voice integration (optional)

You can plug in an external voice assistant that resolves the "active project" by voice and runs Gemini CLI pointed at it. This is entirely optional — without it, Mind works normally with Gemini CLI on the keyboard alone. Details of the general pattern are in `docs/ARQUITETURA.md`, section 8.
